...
- Status: server-authoritative hold-to-interact objective progress is implemented for map objectives and mining nodes; mined resources go into carried loot and client map markers show progress/done state. Extraction now exposes scheduled wave ticks, visible countdown bars, and extraction-zone wave telegraphs. Result stats track kills, objectives, resources, boss damage, extracted/lost loot value, and credits earned. Extraction requirements and side objectives remain.
...
- Status: recipe graph data model, component tags, blueprint gates, source hints, trade rules, boss components, and branch coverage are implemented in the shared catalog; crafting UI now shows category/source/blueprint/trade metadata, missing-component find hints, stable ingredient ordering, a windowed recipe list, and a primitive branch node strip for the selected category. Full zoomable node graph, provenance audit, and persistence-specific unlock tables remain.
...
- Status: station deploy now opens a planet selection screen instead of hardcoding Verdant-9; planet details show threat, biome, resources, boss, hazards, event hook text, and daily/weekly mission panels. Remote queue requests validate planet IDs server-side. Station also has a profile screen with account progression, unlock/cosmetic counts, current loadout, and ship preview. Party readiness, boss rotation timers, and richer missions pages remain.
